...
function Update () {
	...
	//Move to Left
	if((RightTouch.position.x < -0.35) || Input.GetKey(KeyCode.LeftArrow))
	...
	//Move to right
	if((RightTouch.position.x > 0.35) || Input.GetKey(KeyCode.RightArrow))
	{
		if(car != null)
		{
			//Move to Right
	...
	if( (RightTouch.position.y > 0))
	{
		...
		globals.audioPitch = 1;
		//CollisionCamera.transform.position.y -= 2;
	}
	else
	{
		globals.audioPitch = 0.75;
	}
